...
class RequiredParameter(Exception):
    def __init__(self, json_obj, value):
        ...
        self.value = value
    def __str__(self):
        return 'Missing required parameter: \\"' + str(self.value) + '\\" in ' + pretty_json(self.json_obj)
...
def format_parameter_documentation(indent_level, parameter_name, description):
    ...
    wrapped_lines = textwrap.wrap(description, 80-line_start_width-start_description_width)
    line_num = 0
    ret = ''
...
def format_simple_documentation(indent_level, description):
    ...
    comment = '/**'
    description_lines = textwrap.wrap(' ' + description, 80 - len(indent) - len(comment))
    ...
    if 'optional_input_params' in api:
        for optional_params in api['optional_input_params']:
            ...
            if 'description' in optional_params:
                ...
                description = 'Set parameter "' + api_path + '" in API request.'

            ret = ret + format_parameter_documentation(1, cpp_name, description)
            ret = ret + '     */\n'
...
def uses_session_token(api):
    if 'session_token' in api and api['session_token'] == False:
        return False
    return True
...
def delivery_method(api):
    ...
    if 'input_params' in api:
        for (cpp_type, cpp_name, api_path, http_method) in parse_input_params(api['input_params'], api):
            if http_method == 'POST':
                return 'POST'
    if 'optional_input_params' in api:
        for (cpp_type, cpp_name, api_path, http_method) in parse_input_params(api['optional_input_params'], api):
            if http_method == 'POST':
                return 'POST'
...
def get_class_member_vars(api):
    ...
    if 'optional_input_params' in api:
        ...
        for optional_params in api['optional_input_params']:
            ...
            cpp_name = safe_json_get(optional_params, "cpp_name")
            api_path = safe_json_get(optional_params, "api_path")
            lines.append('    boost::optional<' + cpp_type + '> ' + cpp_name + '_;')
        if len(lines) > 0:
            ret = ret + '\n'.join(lines) + '\n'
    return ret
...
def get_as_string_funcs(api):
    ...
    for (enum_cpp_name, enum_members) in parse_enum_params(api['enums']):
        ...
        lines.append('{')
        for (enum_cpp_key, enum_api_value, additional_api_values) in parse_enum_members(enum_members):
            t = '::'.join([version_str, enum_cpp_name, enum_cpp_key])
            lines.append('    if (value == ' + t + ')')
            lines.append('        return "' + enum_api_value + '";')

        lines.append('    return mf::utils::to_string(static_cast<uint32_t>(value));')
        lines.append('}')
        funcs.append( '\n'.join(lines) )
...
def create_content_struct_parse(api, ob, t, full_type_name, n, r, dt, optional, pt):
    ...
    {'''
        ...
        raise UnimplementedError("Parsing struct as array front not implemented")
    else:
        ...
        if enum_cpp_name == t:
            for (enum_cpp_key, enum_api_value, additional_api_values) in parse_enum_members(enum_members):
                ...
                for value in values:
                    ...
                    case = case + '                ' + ob + '->' + n + ' = ' + enum_cpp_name + '::' + enum_cpp_key + ';'
                    cases.append(case)
            ret = ret + '            ' + "\n            else ".join(cases)
            if optional is Required:
                ret = ret + '''
...
def create_content_parse(api, ob, t, n, r, parse_data_type, optional, pt):
    ...
    if parse_data_type is TArray:
        return create_content_parse_array(api, ob, t, n, r, parse_data_type, optional, pt)
    elif parse_data_type is TArrayFront:
        return create_content_parse_array_front(api, ob, t, n, r, parse_data_type, optional, pt)
    elif parse_data_type is TSingle:
        return create_content_parse_single(api, ob, t, n, r, parse_data_type, optional, pt)
    else:
        raise InvalidTType(parse_data_type)

def create_data_type(indent_level, cpp_type, cpp_name, r, parse_data_type, optional, description):
...
def get_data_ctor(api):
    ...
    for ret_data in api['return_params']:
        (cpp_type, cpp_name, api_path, json_type, optional, description) = get_return_parameters(ret_data)
        if is_optional_with_default(optional):
            if cpp_type in enum_names:
                args_with_defaults.append((cpp_name, cpp_type + '::' + optional.default_value))
            else:
...
def get_data_types(api):
    ...
    for ret_data in api['return_params']:
        (cpp_type, cpp_name, api_path, json_type, optional, description) = get_return_parameters(ret_data)
        data_type_sections.append( create_data_type(1, cpp_type, cpp_name, api_path, json_type, optional, description) )
...
def get_data_type_struct(name, param_list):
    ...
    for ret_data in param_list:
        (cpp_type, cpp_name, api_path, json_type, optional, description) = get_return_parameters(ret_data)
        data_type_sections.append( create_data_type(2, cpp_type, cpp_name, api_path, json_type, optional, description) )
    inside = '\n'.join( data_type_sections )
    top = '    struct ' + name + '\n    {\n'
    bottom = '    };'
...
def get_data_type_structs(api):
    ...
    if 'return_structs' not in api or len(api['return_structs']) == 0:
        return ''
    ret = []
    for (struct_cpp_name, struct_members) in parse_return_struct(api['return_structs']):
...
def get_data_type_struct_extractors(api):
    ...
    for (struct_cpp_name, struct_members) in parse_return_struct(api['return_structs']):
        ret.append(get_data_type_struct_extractor(api, struct_cpp_name, struct_members))
...
def content_parse_branch(api, ob, pt, return_param_list):
    ...
    for ret_data in return_param_list:
        ...
        if is_optional_with_default(optional):
            if cpp_type in enum_names:
                ret.append('    ' + ob + '->' + cpp_name + ' = ' + cpp_type + '::' + optional.default_value + ';')
            else:
                ret.append('    ' + ob + '->' + cpp_name + ' = ' + optional.default_value + ';')

    for ret_data in return_param_list:
        (cpp_type, cpp_name, api_path, json_type, optional, description) = get_return_parameters(ret_data)

        if cpp_type in enum_names:
            ret.append(create_content_enum_parse(api, ob, cpp_type, cpp_name, api_path, json_type, optional, pt))

        elif cpp_type in struct_names:
            full_type_name = 'Response::' + cpp_type
            ret.append(
                    create_content_struct_parse(
                        api, ob, cpp_type, full_type_name, cpp_name, api_path, json_type, optional, pt))
        else:
            ret.append(create_content_parse(api, ob, cpp_type, cpp_name, api_path, json_type, optional, pt))
    return '\n'.join(ret)
...
def get_content_parsing(api):
    ...
    if 'return_params' in api:
        ret = ret + content_parse_branch(api, 'response', 'response->pt', api['return_params'])
...
def get_additional_hpp_local_includes(api):
    ...
    if delivery_method(api) == 'POST':
        includes = includes + '#include "mediafire_sdk/http/shared_buffer.hpp"\n'
...
def create_templates(api, target_path):
    ...
    replacements['__API_REQUEST_BASE_HEADER__'] = get_api_request_base_header(api)
    ...
    replacements['__CPP_POST_DATA_IMPL_DECL__'] = get_cpp_post_data_impl_decl(api)
    replacements['__CPP_POST_DATA_IMPL_DEF__'] = get_cpp_post_data_impl_def(api)
    replacements['__CPP_POST_DATA_TEMPLATE__'] = get_cpp_post_data_template(api)
    replacements['__CPP_SESSION_TOKEN_TEMPLATE__'] = get_session_token_api_cpp_template(api)
    ...
    replacements['__DATA_TYPE_STRUCT_EXTRACTORS__'] = get_data_type_struct_extractors(api)
    ...
    replacements['__HPP_SESSION_TOKEN_TEMPLATE__'] = get_session_token_api_hpp_template(api)
    ...
    replacements['__NAMESPACE_DOCUMENTATION__'] = get_namespace_documentation(path_parts_cpp_safe)
            ...
            if key in json and values['analyser'] is not None:
                for child in json[key]:
                    if identifier is None:
                        ...
                        values['analyser'].run(child, child_identifier, ef, duplicate_checker)
                    else:
                        values['analyser'].run(child, None, ef, duplicate_checker)
...
def api_template_enum_analyser():
    ...
    # Required "members"
    members.add_field('cpp_name', is_id = True, required = True, fail_on_duplicate = True)
...
def api_template_return_param_analyser():
    ...
    # Optional fields
    ja.add_field('json_type', permitted_values = ['single', 'array', 'array_front'])
...
def api_template_return_struct_analyser():
    ...
    # Optional "members"
    members.add_field('json_type', permitted_values = ['single', 'array', 'array_front'])
...
def api_template_errors(json):
    ...
    ja.add_array('return_structs', analyser = api_template_return_struct_analyser())
    ja.add_array('optional_input_params', analyser = api_template_optional_input_analyser())
    ja.add_array('return_params', analyser = api_template_return_param_analyser())
