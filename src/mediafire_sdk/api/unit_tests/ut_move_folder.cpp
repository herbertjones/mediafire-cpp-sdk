...
#include "mediafire_sdk/api/actions/move_folder.hpp"
...
#define BOOST_TEST_MODULE MoveFolder
...
BOOST_AUTO_TEST_CASE(UTMoveFolder)
{
    ...
    using MoveFolderType = mf::api::MoveFolder<mf::api::folder::move::Request>;

    using RequestType = MoveFolderType::RequestType;
    using ResponseType = MoveFolderType::ResponseType;

    MoveFolderType::CallbackType HandleMoveFolder = [this, &io_service](
            const ResponseType & response,
            const std::vector<MoveFolderType::ErrorType> & errors)
    {
        if (!errors.empty())
        {
            BOOST_FAIL("HandleMoveFolder reported errors!");
        }
        else
        {
        }
    ...
    auto move_folder
            = MoveFolderType::Create(&stm, "", "", std::move(HandleMoveFolder));
